...
function inventoryView() {
  ...
  const pageStart = (ui.inventoryPage - 1) * INVENTORY_PAGE_SIZE;
  const pageItems = filtered.slice(
    pageStart,
    pageStart + INVENTORY_PAGE_SIZE,
  );
...
function dispatchModal() {
  ...
  return modalShell(
    ...
    `
      <form id="dispatch-form">
        <div class="modal-body">
          <div class="form-grid">
            <div class="form-group full"><label for="dispatch-item">Item</label><select class="field" id="dispatch-item" name="itemId">${itemOptions(eligibleItems, item.id)}</select></div>
            <div class="form-group"><label for="dispatch-quantity">Quantity</label><input class="field" id="dispatch-quantity" name="quantity" type="number" min="1" max="${available(item)}" value="1" required /><span class="help-text" id="available-help">Available: ${formatNumber(available(item))}</span></div>
    ...
    `,
    `<button class="button" data-close>Cancel</button><button class="button primary" type="submit" form="dispatch-form">Confirm dispatch</button>`,
  );
}
